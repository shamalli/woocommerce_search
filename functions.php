...
function wcsearch_heirarhical_dropdowns_menu_init($params) {
	...
	if ($placeholders === false) {
		...
		$placeholders = $taxonomy_obj->labels->name;
	...
	foreach ($chain AS $key=>$parent) {
		...
		if ($terms) {
			...
			echo '<select class="wcsearch-form-control wcsearch-autocomplete ' . esc_attr($functionality) . ' wcsearch-dropdowns-tax-' . esc_attr($tax) . '" data-id="' . esc_attr($uID) . '" data-placeholder="' . esc_attr($chain_placeholder) . '" data-depth-level="' . esc_attr($chain_depth_level) . '" data-tax="' . esc_attr($tax) . '" ' . $autocomplete_data . '>';
			foreach ($terms AS $term) {
				if ($count) {
					$term_count = 'data-count="' . wcsearch_get_count_num(array('term' => $term)) . '"';
				} else {
